...
class MultiBaseModel(nn.Module):
    ...
    def predict(self, x, profile=False, visualize=False, augment=False, embed=None):
        ...
        Returns:
            # (torch.Tensor): The last output of the model.
            return head results: object detection, drivable area, lane detection.
        """
        if augment:
            return self._predict_augment(x)
        return self._predict_once(x, profile, visualize, embed)
    ...
    def _apply(self, fn):
        ...
        self = super()._apply(fn)
        # m = self.model[-1]  # Detect()
        for m in self.model[-3:]:  # Iterate over the last 3 layers
...
class MultiModel(MultiBaseModel):
    """YOLOv8 detection model and seg model."""
    ...
    def _predict_augment(self, x):
        ...
        y = [self._clip_augmented(yij) for yij in yi]  # clip augmented tails
    ...
    def _clip_augmented(self, y):
        """Clip YOLO augmented inference tails."""
        nl = self.model[-1].nl  # number of detection layers (P3-P5)
        g = [sum(4**x for x in range(nl)) for _ in range(nl)]  # grid points
        e = 1  # exclude layer count
        # i = (y[0].shape[-1] // g) * sum(4**x for x in range(e))  # indices
        # y[0] = y[0][..., :-i]  # large
        # i = (y[-1].shape[-1] // g) * sum(4 ** (nl - 1 - x) for x in range(e))  # indices
        ...
        for i in range(len(y)):
            ...
            y[i] = y[i][..., :-indices] if i == 0 else y[i][..., indices:] # large, small
        return y
